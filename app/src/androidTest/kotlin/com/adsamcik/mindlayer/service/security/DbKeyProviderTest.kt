...
/**
 * Instrumented tests for [DbKeyProvider] — exercises real AndroidKeystore + binary-file
 * storage. Tests are hermetic via [clear] in @Before/@After, which wipes the keystore
 * alias, key file, lock file, any quarantined key files, and the throwaway test DB.
 * (The legacy SharedPreferences entry is also cleared so test runs that span the
...
class DbKeyProviderTest {
    ...
    private val legacyPrefFile = "mindlayer_db_key"
    private val keyFile = "mindlayer_db_key.bin"
